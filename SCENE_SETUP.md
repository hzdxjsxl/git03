...
- `lasers`: 激光配置列表
  - `activeTime`: 激光亮起持续时长（毫秒）
  - `inactiveTime`: 激光熄灭持续时长（毫秒），玩家利用此间隙通过
  - `warningTime`: 亮起前预警时长（毫秒），高频闪烁提醒
  - `offsetTime`: 相位偏移（毫秒），错开各激光节奏
  - `angle`: 发射角度
  - `length`: 激光长度
...
### 激光系统
- 三阶段状态机：熄灭(inactive) → 预警(warning) → 亮起(active) → 循环
- 熄灭阶段：2-3.5秒安全窗口，玩家可自由通过
- 预警阶段：1秒高频闪烁（12Hz），发射器和光束交替变暗/变亮
- 亮起阶段：2-2.5秒危险期，碰撞即判定死亡
- 碰撞检测精度: 18像素半径
- 仅亮起阶段判定碰撞，预警/熄灭阶段安全
...
### 问题：游戏不显示
- 检查是否访问了正确的地址 (http://localhost:5173/)
